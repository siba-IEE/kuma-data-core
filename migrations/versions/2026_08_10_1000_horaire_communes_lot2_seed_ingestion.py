"""horaire_communes_lot2_seed_ingestion

Revision ID: 110
Revises: 109
Create Date: 2026-08-10 10:00:00

Chantier profondeur maximale par source, troisieme volet (horaire) - lot 2
sur 4 : series horaires NASA POWER pour **7 communes chef-lieu** (Dubreka,
Faranah, Forecariah, Fria, Gaoual, Gueckedou, Kerouane), fenetre commune
...
revision: str = "110"
down_revision: str | Sequence[str] | None = "109"
...
def upgrade() -> None:
    ...
    if manquantes:
        raise RuntimeError(
            f"Migration 110 : commune(s) introuvable(s) : {sorted(manquantes)}. "
    ...
    if source_id is None:
        raise RuntimeError(f"Migration 110 : source {_SOURCE_CODE!r} introuvable.")
    ...
    if grandeurs_manquantes:
        raise RuntimeError(
            f"Migration 110 : grandeur(s) introuvable(s)/inactive(s) : "
    ...
    if _skip_ingestion_masse():
        op.execute(
            f"-- Migration 110 : ingestion horaire de masse court-circuitee "
    ...
    try:
        ...
        op.execute(
            f"-- Migration 110 : {total} lignes horaires inserees (lot {_NUMERO_LOT}, "
