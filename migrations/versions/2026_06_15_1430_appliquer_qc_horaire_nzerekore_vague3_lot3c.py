"""appliquer_qc_horaire_nzerekore_vague3_lot3c

Revision ID: 065
Revises: 064
Create Date: 2026-06-15 14:30:00.000000+00:00

QC BSRN des lignes horaires brut de Nzerekore
(migration 064). Clone du patron de la migration 057. No-op si 0 ligne
(garde-fou de masse, CI). Clot le rollout des 4 villes neuves.
"""
...
def upgrade() -> None:
    ...
    if not transition_autorisee("brut", "valide_auto"):
        raise RuntimeError("Migration 065 : transition brut -> valide_auto refusee par la matrice.")
    ...
    if ligne_loc is None:
        raise RuntimeError(f"Migration 065 : localite {_LOCALITE_CODE!r} introuvable.")
    ...
    for grandeur in _GRANDEURS:
        ...
        if sid is None:
            raise RuntimeError(
                f"Migration 065 : serie {_code_serie(grandeur)!r} introuvable "
                f"(migration 064 appliquee ?)."
            )
    ...
    try:
        ...
        op.execute(
            f"-- Migration 065 : QC {VERSION_DOCTRINE_QC} applique a {len(verdicts)} lignes "
            f"horaires Nzerekore. {n_valide} valide_auto (dont {n_flag} flaggees), "
