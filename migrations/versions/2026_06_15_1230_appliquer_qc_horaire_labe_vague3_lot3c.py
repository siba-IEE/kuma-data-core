"""appliquer_qc_horaire_labe_vague3_lot3c

Revision ID: 061
Revises: 060
Create Date: 2026-06-15 12:30:00.000000+00:00

QC BSRN des lignes horaires brut de Labe (migration
060). Clone du patron de la migration 057. No-op si 0 ligne (garde-fou de
masse, CI).
"""
...
def upgrade() -> None:
    ...
    if not transition_autorisee("brut", "valide_auto"):
        raise RuntimeError("Migration 061 : transition brut -> valide_auto refusee par la matrice.")
    ...
    if ligne_loc is None:
        raise RuntimeError(f"Migration 061 : localite {_LOCALITE_CODE!r} introuvable.")
    ...
    for grandeur in _GRANDEURS:
        ...
        if sid is None:
            raise RuntimeError(
                f"Migration 061 : serie {_code_serie(grandeur)!r} introuvable "
                f"(migration 060 appliquee ?)."
            )
    ...
    try:
        ...
        op.execute(
            f"-- Migration 061 : QC {VERSION_DOCTRINE_QC} applique a {len(verdicts)} lignes "
            f"horaires Labe. {n_valide} valide_auto (dont {n_flag} flaggees), "
