"""horaire_communes_lot4_seed_ingestion

Revision ID: 114
Revises: 113
Create Date: 2026-08-10 12:00:00

Chantier profondeur maximale par source, troisieme volet (horaire) - lot 4
sur 4 : series horaires NASA POWER pour **7 communes chef-lieu** (Mali,
Mandiana, Pita, Siguiri, Telimele, Tougue, Yomou), fenetre commune
...
def upgrade() -> None:
    ...
    if manquantes:
        raise RuntimeError(
            f"Migration 114 : commune(s) introuvable(s) : {sorted(manquantes)}. "
    ...
    if source_id is None:
        raise RuntimeError(f"Migration 114 : source {_SOURCE_CODE!r} introuvable.")
    ...
    if grandeurs_manquantes:
        raise RuntimeError(
            f"Migration 114 : grandeur(s) introuvable(s)/inactive(s) : "
    ...
    if _skip_ingestion_masse():
        op.execute(
            f"-- Migration 114 : ingestion horaire de masse court-circuitee "
    ...
    try:
        ...
        op.execute(
            f"-- Migration 114 : {total} lignes horaires inserees (lot {_NUMERO_LOT}, "
