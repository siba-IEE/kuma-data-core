"""horaire_communes_lot1_seed_ingestion

Revision ID: 108
Revises: 107
Create Date: 2026-08-10 09:00:00

Chantier profondeur maximale par source, troisieme volet (horaire) - lot 1
sur 4 : series horaires NASA POWER pour **7 communes chef-lieu** (Beyla,
Boffa, Boke, Coyah, Dabola, Dalaba, Dinguiraye), fenetre commune
...
revision: str = "108"
down_revision: str | Sequence[str] | None = "107"
...
def upgrade() -> None:
    ...
    if manquantes:
        raise RuntimeError(
            f"Migration 108 : commune(s) introuvable(s) : {sorted(manquantes)}. "
    ...
    if source_id is None:
        raise RuntimeError(f"Migration 108 : source {_SOURCE_CODE!r} introuvable.")
    ...
    if grandeurs_manquantes:
        raise RuntimeError(
            f"Migration 108 : grandeur(s) introuvable(s)/inactive(s) : "
    ...
    if _skip_ingestion_masse():
        op.execute(
            f"-- Migration 108 : ingestion horaire de masse court-circuitee "
    ...
    try:
        ...
        op.execute(
            f"-- Migration 108 : {total} lignes horaires inserees (lot {_NUMERO_LOT}, "
