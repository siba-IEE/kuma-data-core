...
revision: str = "111"
down_revision: str | Sequence[str] | None = "110"
...
def upgrade() -> None:
    ...
    if not transition_autorisee("brut", "valide_auto"):
        raise RuntimeError("Migration 111 : transition brut -> valide_auto refusee par la matrice.")
    ...
    try:
        ...
        for commune_code in _COMMUNES:
            ...
            if ligne_loc is None:
                raise RuntimeError(f"Migration 111 : localite {commune_code!r} introuvable.")
            ...
            for grandeur in _GRANDEURS:
                ...
                if sid is None:
                    raise RuntimeError(
                        f"Migration 111 : serie {_code_serie(commune_code, grandeur)!r} "
        ...
        session.flush()
        op.execute(
            f"-- Migration 111 : QC {VERSION_DOCTRINE_QC} applique a {total_verdicts} lignes "
