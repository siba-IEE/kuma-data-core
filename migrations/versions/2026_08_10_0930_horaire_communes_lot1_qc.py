"""horaire_communes_lot1_qc

Revision ID: 109
Revises: 108
Create Date: 2026-08-10 09:30:00

Applique le controle qualite algorithmique BSRN (doctrine
``doctrine-qc-horaire.md``, service ``services/qualite/qc_horaire.py``,
pattern des migrations 055/057) aux lignes horaires ``brut`` des 7 communes
du lot 1 (revision precedente).
...
revision: str = "109"
down_revision: str | Sequence[str] | None = "108"
...
def _code_serie(commune_code: str, grandeur: str) -> str:
    return f"{commune_code}_{grandeur}_nasa_power_2001_2025"
...
def upgrade() -> None:
    ...
    if not transition_autorisee("brut", "valide_auto"):
        raise RuntimeError("Migration 109 : transition brut -> valide_auto refusee par la matrice.")
    ...
    try:
        ...
        for commune_code in _COMMUNES:
            ...
            if ligne_loc is None:
                raise RuntimeError(f"Migration 109 : localite {commune_code!r} introuvable.")
            ...
            for grandeur in _GRANDEURS:
                ...
                if sid is None:
                    raise RuntimeError(
                        f"Migration 109 : serie {_code_serie(commune_code, grandeur)!r} "
        ...
        session.flush()
        op.execute(
            f"-- Migration 109 : QC {VERSION_DOCTRINE_QC} applique a {total_verdicts} lignes "
            f"horaires (lot {_NUMERO_LOT}, 7 communes x 9 grandeurs, trigger d'audit "
...
def downgrade() -> None:
    codes_series = [_code_serie(c, g) for c in _COMMUNES for g in _GRANDEURS]
    op.execute(f"ALTER TABLE {_TABLE_MESURES} DISABLE TRIGGER {_TRIGGER_AUDIT}")
    try:
        op.execute(
            sa.text(
