"""appliquer_qc_horaire_pilote_conakry_vague3_lot2

Revision ID: 055
Revises: 054
Create Date: 2026-06-15 09:00:00.000000+00:00

Applique le contrôle qualité algorithmique BSRN
(doctrine ``doctrine-qc-horaire.md`` §4.1-§4.3, socle sourcé verbatim
Long & Dutton) aux 145 000 lignes horaires brut du pilote Conakry
(migration 054).

Verdict par ligne (service ``services/qualite/qc_horaire.py``) :

- tous les tests durs (plausibilité physique) passés -> ``valide_auto``,
  niveau B (transition ``brut -> valide_auto`` validée § 4 statuts) ;
- échecs souples (seuil « extrêmement rare », fermeture, ratio diffus)
  -> ``valide_auto`` + flag tracé en ``commentaire_editorial`` ;
- échec d'un seuil « physiquement impossible » -> ligne **conservée
  ``brut``** + motif de rejet en commentaire (non destructif).

Déterministe : le QC calcule la géométrie solaire localement (``pvlib``,
UTC), aucun appel réseau (pas de ``KUMA_SKIP``). Migration de données
(aucune DDL).

Hors périmètre : tests d'aberration temporelle (doctrine §4.4, différés),
imputation des lacunes, autres villes.
"""
...
def upgrade() -> None:
    ...
    if not transition_autorisee("brut", "valide_auto"):
        raise RuntimeError("Migration 055 : transition brut -> valide_auto refusee par la matrice.")

    # Localite (lat/lon) + series pilote.
    ...
    if ligne_loc is None:
        raise RuntimeError(f"Migration 055 : localite {_LOCALITE_CODE!r} introuvable.")
    ...
    for grandeur in _GRANDEURS:
        ...
        if sid is None:
            raise RuntimeError(
                f"Migration 055 : serie {_code_serie(grandeur)!r} introuvable "
                f"(migration 054 appliquee ?)."
            )
    ...
    try:
        ...
        # 1) Validation en masse de toutes les lignes brut courantes du pilote.
        session.execute(
            sa.text(
                """
        ...
        op.execute(
            f"-- Migration 055 : QC {VERSION_DOCTRINE_QC} applique a {len(verdicts)} lignes "
            f"horaires pilote Conakry. {n_valide} valide_auto (dont {n_flag} flaggees), "
...
def downgrade() -> None:
    # Retour des lignes QC-traitees du pilote a l'etat brut non commente.
