"""seed_et_ingestion_horaire_labe_vague3_lot3c
...
grandeurs) pour Labe (``gin_labe``). Clone du patron de la migration 056,
serie unique pleine profondeur par grandeur. Pixel distinct (aucune
redondance).
...
Note altitude : Labe (1025 m, Fouta-Djalon) est concernee par la
sous-estimation MERRA-2 du gradient d'altitude sur T2M (constatee au
journalier). L'horaire herite du meme caveat ; non bloquant
pour l'ingestion brute.
"""
...
_LOCALITE_CODE: str = "gin_labe"
_VILLE_LIBELLE: str = "Labe"
...
_COMMENTAIRE_EDITORIAL_TEMPLATE: str = (
    ...
    "Statut brut (non validee) : le QC algorithmique horaire (migration 061) "
...
def upgrade() -> None:
    ...
    if ligne_localite is None:
        raise RuntimeError(
            f"Migration 060 : localite {_LOCALITE_CODE!r} introuvable. "
    ...
    if source_id is None:
        raise RuntimeError(
            f"Migration 060 : source_code {_SOURCE_CODE_SQL!r} introuvable. "
    ...
    if grandeurs_manquantes:
        raise RuntimeError(
            f"Migration 060 : grandeur_code(s) introuvable(s) ou inactive(s) : "
    ...
    if _skip_ingestion_masse():
        op.execute(
            f"-- Migration 060 : ingestion de masse court-circuitee "
            f"({_VARIABLE_ENV_SKIP_MASSE}). 6 series Labe seedees, 0 mesure "
    ...
    try:
        ...
        op.execute(
            f"-- Migration 060 : {total} lignes inserees dans "
            f"mesures_ressource_horaires (6 grandeurs x Labe "
