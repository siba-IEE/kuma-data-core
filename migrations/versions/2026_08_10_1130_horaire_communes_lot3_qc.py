...
def upgrade() -> None:
    ...
    if not transition_autorisee("brut", "valide_auto"):
        raise RuntimeError("Migration 113 : transition brut -> valide_auto refusee par la matrice.")
    ...
    try:
        ...
        for commune_code in _COMMUNES:
            ...
            if ligne_loc is None:
                raise RuntimeError(f"Migration 113 : localite {commune_code!r} introuvable.")
            ...
            for grandeur in _GRANDEURS:
                ...
                if sid is None:
                    raise RuntimeError(
                        f"Migration 113 : serie {_code_serie(commune_code, grandeur)!r} "
        ...
        session.flush()
        op.execute(
            f"-- Migration 113 : QC {VERSION_DOCTRINE_QC} applique a {total_verdicts} lignes "
