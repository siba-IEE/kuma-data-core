"""appliquer_qc_horaire_kankan_vague3_lot3c

Revision ID: 063
Revises: 062
Create Date: 2026-06-15 13:30:00.000000+00:00

QC BSRN des lignes horaires brut de Kankan (migration
062). Clone du patron de la migration 057. No-op si 0 ligne (garde-fou de
masse, CI).
"""
...
def upgrade() -> None:
    ...
    if not transition_autorisee("brut", "valide_auto"):
        raise RuntimeError("Migration 063 : transition brut -> valide_auto refusee par la matrice.")
    ...
    if ligne_loc is None:
        raise RuntimeError(f"Migration 063 : localite {_LOCALITE_CODE!r} introuvable.")
    ...
    for grandeur in _GRANDEURS:
        ...
        if sid is None:
            raise RuntimeError(
                f"Migration 063 : serie {_code_serie(grandeur)!r} introuvable "
                f"(migration 062 appliquee ?)."
            )
    ...
    try:
        ...
        op.execute(
            f"-- Migration 063 : QC {VERSION_DOCTRINE_QC} applique a {len(verdicts)} lignes "
            f"horaires Kankan. {n_valide} valide_auto (dont {n_flag} flaggees), "
