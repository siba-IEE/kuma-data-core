"""seed_et_ingestion_horaire_kankan_vague3_lot3c

Revision ID: 062
Revises: 061
Create Date: 2026-06-15 13:00:00.000000+00:00

Ingestion horaire pleine profondeur (2001-2023, 6
grandeurs) pour Kankan (``gin_kankan``). Clone du patron de la migration
056, serie unique pleine profondeur par grandeur. Pixel distinct.
...
_COMMENTAIRE_EDITORIAL_TEMPLATE: str = (
    ...
    "Statut brut (non validee) : le QC algorithmique horaire (migration 063) "
...
def upgrade() -> None:
    ...
    if ligne_localite is None:
        raise RuntimeError(
            f"Migration 062 : localite {_LOCALITE_CODE!r} introuvable. "
    ...
    if source_id is None:
        raise RuntimeError(
            f"Migration 062 : source_code {_SOURCE_CODE_SQL!r} introuvable. "
    ...
    if grandeurs_manquantes:
        raise RuntimeError(
            f"Migration 062 : grandeur_code(s) introuvable(s) ou inactive(s) : "
    ...
    if _skip_ingestion_masse():
        op.execute(
            f"-- Migration 062 : ingestion de masse court-circuitee "
            f"({_VARIABLE_ENV_SKIP_MASSE}). 6 series Kankan seedees, 0 mesure "
    ...
    try:
        ...
        op.execute(
            f"-- Migration 062 : {total} lignes inserees dans "
            f"mesures_ressource_horaires (6 grandeurs x Kankan "
